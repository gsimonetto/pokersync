...
// /planos (client) so' conhece SELF_SERVE_PLAN_IDS, que vive em
// lib/plans/plans-data.ts (dado puro) por causa disso.

// Price ID do Stripe (Dashboard -> Product catalog) por plano -- nenhum
// existe ainda: a conta Stripe esta com os documentos em validacao.
// Assim que a conta liberar e os produtos forem criados, so' preencher
// as env vars abaixo -- checkout e webhook ja estao prontos pra usar.
const PRICE_ENV_BY_PLAN: Partial<Record<PlanId, string>> = {
  individual: "STRIPE_PRICE_INDIVIDUAL",
  individual_pro: "STRIPE_PRICE_INDIVIDUAL_PRO",
